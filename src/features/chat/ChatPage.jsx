        ...
        <div style={{ padding: isMobile ? "10px 12px" : "10px 20px", background: "var(--bg-card)", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: "10px", flexShrink: 0, minWidth: 0, flexWrap: isMobile ? "wrap" : "nowrap" }}>
          {selectedDocument ? (
            <>
              ...
              {/* Redo + Score — icon-only on mobile, labeled on desktop */}
              {chatStarted && !confirmRedo && (
                <>
                  <button onClick={() => setConfirmRedo(true)} title="שיחה חדשה"
                    style={{ ...ghostBtn, padding: isMobile ? "8px 10px" : "6px 12px", minWidth: isMobile ? "40px" : "auto" }}>
                    {isMobile ? "🔄" : "🔄 שיחה חדשה"}
                  </button>
                  <button onClick={handleEndSession} disabled={grading} title="קבל ציון"
                    style={{ ...ghostBtn, background: "var(--brand-light)", color: "var(--brand)", fontWeight: 700, border: "1px solid var(--brand)", padding: isMobile ? "8px 10px" : "6px 12px", minWidth: isMobile ? "40px" : "auto" }}>
                    {grading ? "⏳" : isMobile ? "🏁" : "🏁 קבל ציון"}
                  </button>
                </>
              )}
              ...
              {chatStarted && confirmRedo && (
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  {!isMobile && <span style={{ fontSize: "13px", color: "var(--text-muted)", whiteSpace: "nowrap" }}>למחוק?</span>}
                  <button onClick={handleRedo} style={{ ...ghostBtn, color: "#ef4444", borderColor: "#fca5a5", fontWeight: 700, padding: isMobile ? "8px 10px" : "6px 12px" }}>
                    {isMobile ? "✓" : "כן, מחק"}
                  </button>
                  <button onClick={() => setConfirmRedo(false)} style={{ ...ghostBtn, padding: isMobile ? "8px 10px" : "6px 12px" }}>
                    {isMobile ? "✕" : "ביטול"}
                  </button>
                </div>
              )}
        ...
        <div style={{ flexShrink: 0, padding: isMobile ? "10px" : "14px 16px", background: "var(--bg-card)", borderTop: "1px solid var(--border)", boxShadow: "0 -2px 8px rgba(0,0,0,0.05)" }}>
          ...
          >
            <input
              ...
              dir="rtl"
              style={{ flex: 1, width: "100%", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", padding: isMobile ? "15px 16px" : "13px 16px", fontSize: "14px", background: isInputDisabled ? "var(--bg-page)" : "var(--bg-input)", color: "var(--text-primary)", outline: "none", direction: "rtl", opacity: isInputDisabled ? .6 : 1, fontFamily: "inherit" }}
            ...
            <button type="submit" disabled={isInputDisabled || !inputValue.trim()}
              style={{ background: isInputDisabled || !inputValue.trim() ? "var(--text-muted)" : "var(--brand)", color: "#fff", border: "none", borderRadius: "var(--radius-md)", padding: isMobile ? "15px 18px" : "13px 22px", fontWeight: 700, fontSize: "14px", cursor: isInputDisabled || !inputValue.trim() ? "not-allowed" : "pointer", whiteSpace: "nowrap", flexShrink: 0, fontFamily: "inherit", width: isMobile ? "100%" : "auto" }}>
              {loading ? "..." : "שלח 🚀"}
            </button>
          </form>
          {isEducatorMode && chatStarted && messages.length > 0 && !isMobile && (
...
function MessageBubble({ message }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: isBot ? "row" : "row-reverse", alignItems: "flex-end", gap: "10px" }}>
      ...
      <div style={{ maxWidth: isMobile ? "90%" : "75%", padding: isMobile ? "10px 14px" : "12px 16px", borderRadius: isBot ? "16px 16px 16px 4px" : "16px 16px 4px 16px", background: isBot ? "var(--msg-bot-bg)" : "var(--msg-user-bg)", color: isBot ? "var(--msg-bot-text)" : "var(--msg-user-text)", border: isBot ? "1px solid var(--border)" : "none", direction: "rtl", textAlign: "right", wordBreak: "break-word" }}>
        {isBot
